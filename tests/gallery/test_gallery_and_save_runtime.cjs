...
function makeSaveRuntime() {
  ...
  const applyFilters = ["masked", "unmasked", "reviewed", "unreviewed"].map((name) => {
    const input = nodes.get(`#applyFilter${name[0].toUpperCase()}${name.slice(1)}`);
    input.dataset.applyImageFilter = name; nodes.set(`[data-apply-image-filter="${name}"]`, input); return input;
  });
  nodes.get("#applyDivisor").value = "16"; nodes.get("#divisor").value = "16"; nodes.get("#applySuffix").value = "_m";
  ...
  const state = {
    ...
    settings: { saving: { default_output_directory: "G:/out", parallelism: 1, preserve_directory_structure: true, image_filters: [] }, detection: { exclude_forced_default: true }, confirmations: { overwriteSource: false, deleteSourceAfterCopy: false } }, drafts: new Map(), maskStatus: new Map(), selectedImageIds: new Set(), candidateUpdateChains: new Map(),
  ...
  const context = {
    ...
    processableImages() { return state.images.filter((image) => !image.hidden); }, isHidden(image) { return Boolean(image?.hidden); }, isReviewed(image) { return Boolean(image?.reviewed); }, imageHasMask(image) { return Boolean(image?.masked); }, imageMatchesStateFilter(image, filters) { return !filters.size || (filters.has("masked") && Boolean(image.masked)) || (filters.has("unmasked") && !image.masked) || (filters.has("reviewed") && Boolean(image.reviewed)) || (filters.has("unreviewed") && !image.reviewed); }, syncDetectionTargetSwitch() {}, isBusy() { return Boolean(context.busy || state.saveStarting || state.saving); }, currentImageActionPending() { return Boolean(state.pendingImageId); }, catalogStagingEditsActive() { return false; }, isProcessableImage(image) { return Boolean(image && !image.hidden); }, async flushDraftSaves() { if (context.flushError) throw context.flushError; }, async flushWorkspaceDraft(imageId) { calls.push(`flush:${imageId}`); }, async waitForCandidateMutations() { calls.push("wait-candidates"); }, updateBlockSizeDisplay() { calls.push("block-size"); },
...
async function saveInteractions() {
  ...
  runtime.refreshApplyTargets();
  await runtime.openApplyDialog();
  assert.deepEqual(Array.from(runtime.applyImageFilters()), ["reviewed"], "batch-save filters are retained while this page stays open");
  ...
  assert.deepEqual(state.settings.saving, { default_output_directory: "G:/directory-first", parallelism: 1, preserve_directory_structure: false, image_filters: [] }, "a late structure reply cannot replace a newly committed output directory");
  ...
  assert.deepEqual(state.settings.saving, { default_output_directory: "G:/directory-last", parallelism: 1, preserve_directory_structure: true, image_filters: [] }, "a late directory reply cannot replace a newly committed structure preference");
  ...
  state.images = [{ id: "masked", masked: true }, { id: "unmasked", masked: false }, { id: "hidden", masked: true, hidden: true }]; runtime.setApplyImageFilters(["masked"]); runtime.refreshApplyTargets(); assert.deepEqual(Array.from(state.applyTargetIds), ["masked"]);
  ...
  state.applyTargetIds = []; await runtime.openApplyDialog(); state.applyTargetIds = ["file"]; await runtime.openApplyDialog(); assert.equal(runtime.nodes.get("#applyDialog").open, true);
