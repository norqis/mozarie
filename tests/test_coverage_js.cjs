...
nodeTest("frontend shard coverage merges every Node map and available browser V8 input", async () => {
  ...
  try {
    ...
    for (const shard of shards) {
      ...
      fs.writeFileSync(path.join(shard, "node", "coverage-final.json"), "{}", "utf8");
    }
    fs.writeFileSync(path.join(shards[0], "browser-v8.json"), "[{}]", "utf8");
    let verified = false;
    let written = false;
    await mergeFrontendShardCoverage(shards, path.join(temporaryRoot, "report"), {
      async browserCoverageMap(entries) { assert.deepEqual(entries, [{}]); return createCoverageMap({}); },
      verifyCoverage(map) { assert.equal(typeof map.files, "function"); verified = true; },
    ...
    fs.writeFileSync(path.join(shards[1], "node", "coverage-final.json"), "{", "utf8");
    await assert.rejects(mergeFrontendShardCoverage(shards, path.join(temporaryRoot, "bad-report"), {
      async browserCoverageMap() { return createCoverageMap({}); },
      verifyCoverage() {},
