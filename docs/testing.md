...
`manual` には、CIで所有できない具体的な環境と、人がそこで観測する理由を書く。「実機」「環境依存」「目視」などの抽象語だけでは登録できない。元文書の第一列には元IDではなく残存する観測keyを一行ずつ記載し、いずれかのセルにJSONの `observation` を完全一致で記載する。`automated` と `retired` の観測行は元文書から削除する。元文書の残存key・観測と台帳が1対1で一致しない場合も失敗にする。OS所有の画面、実GPU・実モデル、物理I/O障害、主観を含む操作感なども、決定的な規則を先に自動化し、残った観測だけを `manual` とする。

確認項目を削除できるのは、同じ利用者観測を行うテストが通常実行とCIの双方で収集・実行され、passしたことをmanifestで照合できる場合に限る。製品仕様から廃止した項目は `retired` とし、理由が分かる製品変更と同じ差分で扱う。契約JSON、対応するテスト、確認文書の削除は同じコミットに含める。
